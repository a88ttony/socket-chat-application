...
def large_port(large_item, small_item):
    ...
    print("large2: %d" % large_port)
    print("small2: %d" % small_port)
    top = tkinter.Tk()
    top.title("Chatting with - "+small_user)
    ...
    def client():
        ...
        def send(event=None):  
            ...
            msg_list.see(tkinter.END)
            message = {"number": i, "name": large_user, "chat": my_msg.get()}
            s.sendto(my_msg.get().encode('utf-8'), ('127.0.0.1', small_port))
            i = i+1
            my_msg.set("")  
    ...
    def server():
        ...
        global message
        s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        s.bind(('127.0.0.1', large_port))
        s.setblocking(0) 
        while True:
            try:
                data, addr = s.recvfrom(1024)
                msg = small_user+": "+data.decode('utf-8')
                msg_list.insert(tkinter.END, msg)
                msg_list.see(tkinter.END)
                time.sleep(1)
            except:
    ...
    def send_file():
        if(small_item['state'] == 0):
            msg = small_user+" is not available now!"
            msg_list.insert(tkinter.END, msg)
        elif(small_item['state'] == 1):    
            s = socket.socket(socket.AF_INET,socket.SOCK_STREAM)
            s.connect(('127.0.0.1', smallfile_port))   
            while True:
                root = tkinter.Tk()
                root.withdraw()
